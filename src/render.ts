...
export function renderCards(docs: Course[], container: HTMLElement) {
  ...
  docs.forEach((d) => {
    ...
    cardHeader.className = `card-header bg-${courseStatus} text-bg-${courseStatus} d-flex justify-content-between gap-1`;
      ...
      shareCourseLink.onclick = async (e) => {
        ...
        try {
          await navigator.share({
            ...
            text: `Oferta de ${d.cargo} en ${d.escuela} (${d.descdistrito} | ${d.descnivelmodalidad}). Cierre de oferta: ${d.finoferta ? dateTimeFormatter.format(new Date(d.finoferta)) : "No especificado"}. Más info en el sitio de Actos Públicos Digitales.`,
